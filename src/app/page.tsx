...
const Card: React.FC<{ title: string; description: string; content: string }> = ({ title, description, content }) => (
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-r from-purple-400 to-pink-500">
      ...
      <header className="bg-white shadow-md fixed top-0 left-0 right-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div>
            ...
            <Button onClick={() => router.push('/signup')} className="bg-purple-600 text-white hover:bg-purple-700">
              Sign Up
            </Button>
          </div>
        </div>
      ...
      <main className="flex-grow flex flex-col items-center justify-center pt-20 px-4">
        <h1 className="text-6xl font-bold text-white text-center mb-4">Bid Master</h1>
        <p className="text-xl text-white italic mb-8 max-w-lg text-center">
          &quot;Instead of interrupting, work on attracting.&quot;
        </p>
        <Button onClick={() => router.push('/signup')} className="bg-white text-purple-600 hover:bg-gray-100 font-semibold text-lg mb-16">
          Get Started
        </Button>
